...
module.exports = function (config) {
  ...
  function getUpdateAuthQuestions ({ defaultUrl }) {
    return [
      { type: 'input', name: 'url', message: 'Jira URL: ', default: defaultUrl },
      { type: 'input', name: 'pass', message: 'Password/API token: ', validate: (input) => input.length > 0 }
    ];
  }
  ...
  return {
    ...
    update (options) {
      inquirer.prompt(
        getUpdateAuthQuestions(config.auth.url)
      ).then(answer => {
        answer.url = answer.url || config.auth.url;
        answer.user = answer.user || config.auth.user;
        const { err } = this.updateConfig(answer, options);
        if (err) {
          console.log(err);
          return err;
